...
if ! tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
  echo "❌ tmux session '$SESSION_NAME' not found. Please run start_control.sh first."
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "❌ 'foxglove' window not found. Please run control_only.sh first."
  exit 1
fi

echo "🚀 Starting Air camera system..."

# Create and start 'camera' window
...
echo "✅ Camera launched successfully."
