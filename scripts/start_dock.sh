...
tmux send-keys -t "$SESSION_NAME:$WINDOW_NAME" 'docker exec -it beautiful_bartik bash -c "
  ...
  ros2 action send_goal /dock_robot opennav_docking_msgs/action/DockRobot \"{use_dock_id: true, dock_id: '\''home_dock'\'', dock_type: '\''simple_charging_dock'\''}\"
"' C-m

echo "ℹ️ Docking started with:"
echo "ℹ️ ros2 action send_goal /dock_robot opennav_docking_msgs/action/DockRobot \"{use_dock_id: true, dock_id: 'home_dock', dock_type: 'simple_charging_dock'}\""
